		...
		if bodie.has_method("_damage") and bodie != self:
			if "durability" in bodie:
				if bodie.durability > 0:
					...
					bodie._damage(80 - explosion_distance);
					
		if bodie.has_method("get_hurt"):
	#		if "damage" in player_data:
#				if bodie.current_health > 0:
#					var explosion_distance = (5 * player_data.global_transform.origin.distance_to(global_transform.origin));
					bodie.get_hurt(4);
