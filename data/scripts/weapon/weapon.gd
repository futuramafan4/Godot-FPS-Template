class weapon:
	...
	var weapont = load("res://data/scenes/Resources.gd");
	
	#onready var resources = get_node("/root/World/Player/Resources")
#	onready var resources = get_node("/root/L_Main/character/Resources")
	
	func _init(owner, name, firerate, ammo, damage, reload_speed) -> void:
		self.owner = owner;
		self.name = name;
	...
	func _shoot(_delta) -> void:
		...
		if animc != "Reload" and animc != "Draw" and animc != "Hide":
				...
				owner.camera.rotation.x = lerp(owner.camera.rotation.x, rand_range(.25, .5), _delta);
				owner.camera.rotation.y = lerp(owner.camera.rotation.y, rand_range(-.25, .25), _delta);
...
#	func gain_ammo(amount):
#		if ammo < max_bullets:
#			bullets += amount
#			bullets = int(clamp(bullets, 0, max_bullets))
##		current_ammo[weapon_type] += amount
##		current_ammo[weapon_type] = clamp(current_ammo[weapon_type],0,max_ammo[weapon_type])
#		return true 
