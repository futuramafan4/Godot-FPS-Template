...
const TURRET_DAMAGE_BULLET = 2
const TURRET_DAMAGE_RAYCAST = 2
	...
	# Otherwise we are using raycasting (which works most of the time, sometimes it does not for some reason)
		...
		if node_raycast.is_colliding():
			...
			if body.has_method("get_hurt"):
				body.get_hurt(TURRET_DAMAGE_RAYCAST, node_raycast.global_transform)
